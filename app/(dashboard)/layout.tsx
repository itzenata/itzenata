    ...
    <Providers>
      <main className="flex min-h-screen w-full flex-col bg-transparent">
        ...
        <div className="flex flex-col sm:gap-4 sm:py-4 sm:pl-14">
          ...
          <main className="mt-23 grid flex-1 items-start gap-2 p-4 sm:px-6 sm:py-0 md:gap-4 bg-muted/40 bg-white">
            {children}
          </main>
        </div>
        <Analytics />
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-10 hidden w-14 flex-col border-r bg-background sm:flex">
      <nav className="flex flex-col items-center gap-4 px-2 sm:py-5">
        ...
        <NavItem href="/" label="Entreprises">
          <Building className="h-5 w-5" />
        </NavItem>
        <NavItem href="/accounts" label="Plan comptable">
          <ClipboardList className="h-5 w-5" />
        </NavItem>
        <NavItem href="/accountsTiers" label="Entreprises">
